...
azkaban.FlowExecuteDialogView = Backbone.View.extend({
  ...
  getExecutionOptionData: function () {
    ...
    var executingData = {
      ...
      ajax: "modifyAndRetryFlowInstance",
      flow: this.flowId,
...
var exGraphModel = new azkaban.GraphModel();
function getExGraph() {
  var requestURL;
  var requestData;
  ...
  ajaxCall(requestURL, requestData, function (data) {
    console.log("data fetched");
    exGraphModel.addFlow(data);
    exGraphModel.trigger("change:graph");
  });
}
getExGraph();
